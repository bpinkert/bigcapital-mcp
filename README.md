...
affiliated with, endorsed by, or supported by Bigcapital Technologies.
Bigcapital is a separate project licensed under AGPL-3.0; this server
communicates with it only over its public HTTP API and contains no Bigcapital
application source code. The bundled `openapi.json` is a patched copy of the
API specification published in the
[Bigcapital repository](https://github.com/bigcapitalhq/bigcapital) and remains
subject to that project's AGPL-3.0 terms.

The `server/` code is generated by
